...
for cur_iter in range(max_iter):
    for i,x in enumerate(X):
        ...
        nxsum += 1
        Z[i] = z
    A[cur_iter] =(nz[1] + alpha)/(nxsum + K * alpha)
    B[cur_iter] =(nxz[1][1] + beta)/(nz[1] +  V * beta)
    C[cur_iter] =(nxz[1][0] + beta)/(nz[0] +  V * beta)
#%%
print(np.mean(A[-300:])) #硬币A正面朝上的概率
print(np.mean(B[-300:])) #硬币B正面朝上的概率
print(np.mean(C[-300:])) #硬币C正面朝上的概率
